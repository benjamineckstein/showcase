...
class DesignRulesTest {
    ...
    void controller_annotated_with_restcontroller() {
        classes().that().haveNameMatching(".*[cC]ontroller").should().beAnnotatedWith(RestController.class).check(SHOWCASECLASSES);
    }


    @Test
    void controller_naming_pattern() {
        classes().that().areAnnotatedWith(RestController.class).should().haveNameMatching(".*[cC]ontroller").check(SHOWCASECLASSES);
    }

    @Test
    void controller_returns_responseentity() {
        classes().that().areAnnotatedWith(RestController.class).should(new ArchCondition<>("only have methods returning Responseentity") {
            @Override
            public void check(JavaClass item, ConditionEvents events) {
                item.getMethods().stream()
                        ...
                        .forEach(m -> {
                            String message = String.format("Method %s.%s is returning %s instead of ResponseEntity", item.getName(), m.reflect().getName(), m.getRawReturnType().getSimpleName());
    ...
    void boundary_methods_transactional() {
        classes().that().areAnnotatedWith(Boundary.class).should(new ArchCondition<>("only have methods annotated with @Transactional") {
            @Override
            public void check(JavaClass item, ConditionEvents events) {
                item.getMethods().stream()
                        .filter(isMethodPublicAndNotSynthetic())
